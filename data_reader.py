...
def merge(filePath=config.dataset_name+'/denovo'):
    # print('///'*32)    ABRF_DDA/DDPresult
    fileList = os.listdir(filePath)
    # print(fileList)
    pdList = []
    for i in fileList:
        print(filePath+os.sep+i)
        pdList.append(pd.read_csv(filePath+os.sep+i, sep='\t'))
    res = pd.concat(pdList, ignore_index=True)
    res.drop_duplicates(subset=['feature_id'], keep='first', inplace=True)
    res.to_csv(config.denovo_output_file, sep='\t', index=False)
    print('MergeFile has been completed.')
    # print('ABRF_DDA/'+fileList[0][:-1])
...
def del_file(filePath):
    if os.path.exists(filePath):
        ...
        print(f"The {filePath} folder has been emptied")
...
class BaseDataset(Dataset):
    def __init__(self, feature_filename, spectrum_filename, transform=None):
        ...
        with open(feature_filename, 'r') as fr:
            ...
            for line in reader:
                mass = (float(line[mz_index]) -
                        config.mass_H) * float(line[z_index])
    ...
    def _get_feature(self, feature: DDAFeature):
        raise NotImplementedError(
            "subclass should implement _get_feature method")
...
class DeepNovoTrainDataset(BaseDataset):
    def _get_feature(self, feature: DDAFeature) -> TrainData:
        ...
        peak_location, peak_intensity, spectrum_representation = process_peaks(
            mz_list, intensity_list, feature.mass)
...
def collate_func(train_data_list):
    ...
    spectrum_representation = [
        x.spectrum_representation for x in train_data_list]
    spectrum_representation = np.stack(
        spectrum_representation)  # [batch_size, embed_size]
    ...
    batch_forward_id_target = torch.from_numpy(
        np.stack(batch_forward_id_target))  # [batch_size, T]
    batch_forward_ion_index = torch.from_numpy(
        np.stack(batch_forward_ion_index))  # [batch, T, 26, 8]
    ...
    batch_backward_id_target = torch.from_numpy(
        np.stack(batch_backward_id_target))  # [batch_size, T]
    batch_backward_ion_index = torch.from_numpy(
        np.stack(batch_backward_ion_index))  # [batch, T, 26, 8]
    batch_backward_id_input = torch.from_numpy(
        np.stack(batch_backward_id_input))
...
def denovo_collate_func(data_list: List[_DenovoData]):
    ...
    batch_spectrum_representation = np.array(
        [x.spectrum_representation for x in data_list])

    batch_peak_location = torch.from_numpy(batch_peak_location)
    batch_peak_intensity = torch.from_numpy(batch_peak_intensity)
    batch_spectrum_representation = torch.from_numpy(
        batch_spectrum_representation)
...
class DBSearchDataset(BaseDataset):
    def __init__(self, feature_filename, spectrum_filename, db_searcher: DataBaseSearcher):
        super(DBSearchDataset, self).__init__(
            feature_filename, spectrum_filename)
        ...
        if config.quick_scorer == "num_matched_ions":
            ...
            raise ValueError(
                f"unknown quick_scorer attribute: {config.quick_scorer}")
    ...
    def peptide_to_aa_id_seq(peptide: list, direction=0):
        ...
        if len(peptide) > config.MAX_LEN - 2:
            raise ValueError(
                f"received a peptide longer than {config.MAX_LEN}")
    ...
    def _get_feature(self, feature: DDAFeature):
        ...
        if not mz_list:
            return None
        peak_location, peak_intensity, _ = process_peaks(
            mz_list, intensity_list, feature.mass)
        precursor_mass = feature.mass

        candidate_list = self.db_searcher.search_peptide_by_mass(
            precursor_mass, pad_with_random_permutation=True)
        ...
        if len(candidate_list) > config.normalizing_std_n:
            quick_scores = [self.quick_scorer(
                feature.mass, peak_location, peak_intensity, pc.seq) for pc in candidate_list]
            quick_scores = np.array(quick_scores)
            top_k_ind = np.argpartition(
                quick_scores, -config.normalizing_std_n)[-config.normalizing_std_n:]
        ...
        for pc in candidate_list:
            ...
            forward_ion_location_index_arr.append(
                forward_ion_location_index_list)
            backward_ion_location_index_arr.append(
                backward_ion_location_index_list)  # nested_list
        ...
        if batch_max_seq_len > 50:
            logger.warning(
                f"feature {feature} has sequence candidate longer than 50")
    ...
    def get_num_matched_fragment_ions(cls, precursor_mass, peaks_location, peaks_intensity, seq):
        ...
        for i, id in enumerate(forward_id_input):
            prefix_mass += config.mass_ID[id]
            ion_location = cls.get_fragment_ion_location(
                precursor_mass, prefix_mass)  # [3]
    ...
    def peaks_quick_scorer(cls, precursor_mass, peaks_location, peaks_intensity, seq):
        ...
        for i, id in enumerate(forward_id_input):
            ...
            # each observed peak can only be explained by one ion type
            score_vec = np.max(np.exp(-np.square(mz_diff*100)), axis=1)
