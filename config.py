...
parser.add_argument("--search_denovo",
                    dest="search_denovo", action="store_true")
...
parser.set_defaults(local_rank=-1)
...
def var_mod_peptide_transform(peptide: list):
    ...
    :return:
    """
    position_list = [position for position,
                     aa in enumerate(peptide) if aa in var_mod_dict]
...
mass_AA_min_round = int(
    round(mass_AA_min * KNAPSACK_AA_RESOLUTION))  # 57.02146
...
input_feature_file_valid = dataset_name+"/features.csv.identified.valid.nodup"
...
# denovo files
denovo_input_spectrum_file = dataset_name+"/spectrums.mgf"
denovo_input_feature_file = dataset_name+"/features.csv.identified.test.nodup"
...
# CUDA_VISIBLE_DEVICES="0,2,4,5,6,7" python -m torch.distributed.launch --nproc_per_node 6 main.py
