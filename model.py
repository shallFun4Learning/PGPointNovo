...
class TNet(nn.Module):
    ...
    def __init__(self, with_lstm=False):
        ...
        self.conv1 = nn.Conv1d(config.vocab_size *
                               config.num_ion + 1, num_units, 1)
        ...
        self.input_batch_norm = nn.BatchNorm1d(
            config.vocab_size * config.num_ion + 1)
...
class DeepNovoPointNet(nn.Module):
    ...
    def forward(self, location_index, peaks_location, peaks_intensity):
        ...
        # [batch, T, N, 1]
        peaks_location = peaks_location.expand(-1, T, -1, -1)
        peaks_location_mask = (peaks_location > 1e-5).float()
        # [batch, T, N, 1]
        peaks_intensity = peaks_intensity.expand(-1, T, -1, -1)

        location_index = location_index.view(
            batch_size, T, 1, vocab_size * num_ion)
        ...
        location_exp_minus_abs_diff = location_exp_minus_abs_diff * \
            peaks_location_mask * location_index_mask

        input_feature = torch.cat(
            (location_exp_minus_abs_diff, peaks_intensity), dim=3)
        input_feature = input_feature.view(
            batch_size * T, N, vocab_size * num_ion + 1)
...
class InitNet(nn.Module):
    def __init__(self):
        ...
        self.init_state_layer = nn.Linear(
            config.embedding_size, 2 * config.lstm_hidden_units)
...
class DeepNovoPointNetWithLSTM(nn.Module):
    ...
    def forward(self, location_index, peaks_location, peaks_intensity, aa_input=None, state_tuple=None):
        ...
        ion_feature = self.t_net(input_feature).view(
            batch_size, T, config.num_units)  # attention on peaks
        ...
        output_feature = torch.cat(
            (ion_feature, activation_func(output_feature)), dim=2)
    ...
    def step(self, candidate_location, peaks_location, peaks_intensity, aa_input, state_tuple, direction):
        ...
        with torch.no_grad():
            if config.use_lstm:
                ...
                logit = model(candidate_location,
                              peaks_location, peaks_intensity)
...
# _get_ion_index_device = device
try:
    mass_ID_torch = torch.from_numpy(config.mass_ID_np).to(
        _get_ion_index_device)
except:
    print('///'*3+f'error:{_get_ion_index_device} '+'///'*3)
    raise
mass_ID_torch.unsqueeze(0)
...
def torch_get_ion_index(peptide_mass: List[float], prefix_mass: List[float], direction):
    ...
    """
    peptide_mass = torch.tensor(
        peptide_mass, dtype=torch.float32, device=_get_ion_index_device).unsqueeze(1)  # batch
    prefix_mass = torch.tensor(
        prefix_mass, dtype=torch.float32, device=_get_ion_index_device).unsqueeze(1)  # batch
    with torch.no_grad():
        if direction == 0:
        ...
        # [batch, 26, 12], out of bound index would have value 0
