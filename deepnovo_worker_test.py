...
class WorkerTest(object):
    ...
    def test_accuracy(self, db_peptide_list=None):
        ...
        target_len_db_mass = sum([len(x)
                                 for x in target_dict_db_mass.values()])
        ...
        for index, predicted in enumerate(self.predicted_list):
            ...
            if feature_id in target_dict_db_mass:
                ...
                for predicted_sequence, predicted_score in zip(predicted["sequence"], predicted["score"]):
                    predicted_AA_id = [config.vocab[x]
                                       for x in predicted_sequence]
                    target_AA_id = [config.vocab[x] for x in target]
                    recall_AA = self._match_AA_novor(
                ...
                if predicted["score"]:
                    predicted_score_max = '{0:.2f}'.format(
                        np.max(predicted["score"]))
                else:
                    predicted_score_max = ''
                print_list = [feature_id,
                              feature_area,
            ...
            if feature_count > 1:
                for feature_id in feature_list:
                    if feature_id in multifea_dict:
                        multifea_dict[feature_id].append(
                            scan_id + ':' + str(feature_count))
                    else:
                        multifea_dict[feature_id] = [
                            scan_id + ':' + str(feature_count)]
        ...
        print("recall_AA_total = {0:.4f}".format(
            recall_AA_total / target_len_total))
        print("recall_AA_db = {0:.4f}".format(recall_AA_total / target_len_db))
        print("recall_AA_db_mass = {0:.4f}".format(
            recall_AA_total / target_len_db_mass))
        print("recall_peptide_total = {0:.4f}".format(
            recall_peptide_total / target_count_total))
        print("recall_peptide_db = {0:.4f}".format(
            recall_peptide_total / target_count_db))
        print("recall_peptide_db_mass = {0:.4f}".format(
            recall_peptide_total / target_count_db_mass))
        print("precision_AA_mass_db  = {0:.4f}".format(
            recall_AA_total / predicted_len_mass_db))
        print("precision_peptide_mass_db  = {0:.4f}".format(
            recall_peptide_total / predicted_count_mass_db))

        # record
        with open('results/rec', "a+") as rec:
            print("target_count_total = {0:d}".format(
                target_count_total), file=rec)
            print("target_len_total = {0:d}".format(
                target_len_total), file=rec)
            print("target_count_db = {0:d}".format(target_count_db), file=rec)
            print("target_len_db = {0:d}".format(target_len_db), file=rec)
            print("target_count_db_mass: {0:d}".format(
                target_count_db_mass), file=rec)
            print("target_len_db_mass: {0:d}".format(
                target_len_db_mass), file=rec)
            print()

            print("predicted_count_mass: {0:d}".format(
                predicted_count_mass), file=rec)
            print("predicted_count_mass_db: {0:d}".format(
                predicted_count_mass_db), file=rec)
            print("predicted_len_mass_db: {0:d}".format(
                predicted_len_mass_db), file=rec)
            print("predicted_only: {0:d}".format(predicted_only), file=rec)
            print()

            print("recall_AA_total = {0:.4f}".format(
                recall_AA_total / target_len_total), file=rec)
            print("recall_AA_db = {0:.4f}".format(
                recall_AA_total / target_len_db), file=rec)
            print("recall_AA_db_mass = {0:.4f}".format(
                recall_AA_total / target_len_db_mass), file=rec)
            print("recall_peptide_total = {0:.4f}".format(
                recall_peptide_total / target_count_total), file=rec)
            print("recall_peptide_db = {0:.4f}".format(
                recall_peptide_total / target_count_db), file=rec)
            print("recall_peptide_db_mass = {0:.4f}".format(
                recall_peptide_total / target_count_db_mass), file=rec)
            print("precision_AA_mass_db  = {0:.4f}".format(
                recall_AA_total / predicted_len_mass_db), file=rec)
            print("precision_peptide_mass_db  = {0:.4f}".format(
                recall_peptide_total / predicted_count_mass_db), file=rec)
            print('='*60, file=rec)
    ...
    def _get_predicted_peaks(self):
        ...
        with open(self.predicted_file, 'r') as handle:
            ...
            for line in handle:
                ...
                predicted["feature_id"] = "F" + \
                    line_split[col_fraction_id] + ":" + line_split[col_scan_id]
