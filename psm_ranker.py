...
def wrap_infer_data(value_net_infer_data: DBSearchData):
    ...
    :return:
    """
    peak_location = torch.from_numpy(
        value_net_infer_data.peak_location).to(device)
    peak_intensity = torch.from_numpy(
        value_net_infer_data.peak_intensity).to(device)
    forward_id_target = torch.from_numpy(
        value_net_infer_data.forward_id_target).to(device)
    backward_id_target = torch.from_numpy(
        value_net_infer_data.backward_id_target).to(device)
    forward_ion_location_index = torch.from_numpy(
        value_net_infer_data.forward_ion_location_index).to(device)
    backward_ion_location_index = torch.from_numpy(
        value_net_infer_data.backward_ion_location_index).to(device)
    ...
    precursor_mass = torch.tensor(
        value_net_infer_data.precursor_mass).to(device)
    ...
    def search(self):
        ...
        with torch.no_grad():
            for i, value_net_infer_data in enumerate(self.data_reader):
                ...
                if i % 100 == 0:
                    logger.info(
                        f"searching for {i}th/{self.num_spectra} spectra")
                ...
                for i in range(forward_id_target.size(0) // config.inference_value_max_batch_size):
                    ...
                    forward_masking_matrix = ~(
                        temp_forward_id_target == config.PAD_ID)
                    ...
                    forward_logprob = torch.sum(
                        forward_logprob, dim=1)  # (num_candidate,)
                    ...
                    backward_masking_matrix = ~(
                        temp_backward_id_target == config.PAD_ID)
                    ...
                    backward_logprob = torch.sum(
                        backward_logprob * backward_masking_matrix, dim=1)  # (num_candidate,)

                    peptide_log_prob.append(
                        torch.max(forward_logprob, backward_logprob))
                ...
                if forward_id_target.size(0) - (i + 1) * config.inference_value_max_batch_size > 0:
                    temp_forward = self.forward_model(
                        forward_ion_location_index[(
                            i + 1) * config.inference_value_max_batch_size:],
                        peaks_location[(i + 1) *
                                       config.inference_value_max_batch_size:],
                        peaks_intensity[(i + 1) *
                                        config.inference_value_max_batch_size:]
                    )  # (num_candidate, T, 26)
                    temp_forward_id_target = forward_id_target[(
                        i + 1) * config.inference_value_max_batch_size:]
                    ...
                    forward_logprob = torch.gather(F.log_softmax(temp_forward, dim=-1),
                                                   ...
                                                   index=temp_forward_id_target.unsqueeze(-1)).squeeze(2)
                    forward_logprob = torch.sum(
                        forward_logprob * forward_masking_matrix, dim=1)  # (num_candidate,)
                ...
                log_length_normalized_score = peptide_log_prob / \
                    torch.log(aa_length)
                ...
                length_normalize_mu, length_normalize_sigma = get_normalization_stats(
                ...
                for i, index in enumerate(top_n_psm_indices):
                    ...
                    length_score = float(
                        length_normalized_score[valid_indices][index])
                    log_length_score = float(
                        log_length_normalized_score[valid_indices][index])
                    ...
                    current_psm = PSM(feature_id=value_net_infer_data.dda_feature.feature_id,
                                      ...
                                      is_decoy=self.accession_id_is_decoy(
