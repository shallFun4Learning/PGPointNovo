...
class KnapsackSearcher(object):
    def __init__(self, MZ_MAX, knapsack_file):
        ...
        if os.path.isfile(knapsack_file):
            ...
            logging.info(
                "KnapsackSearcher.__init__(): build knapsack matrix from scratch")
    ...
    def _build_knapsack(self):
        ...
        knapsack_matrix = np.zeros(
            shape=(config.vocab_size, max_mass_upperbound), dtype=bool)
        for aa_id in range(3, config.vocab_size):
            mass_aa = int(
                round(config.mass_ID[aa_id] * self.knapsack_aa_resolution))
...
class SearchPath:
    ...
    score_list: list
    score_sum: float
    # state tupe store in search path is of shape [num_lstm_layers, num_units]
    lstm_state: tuple
    direction: Direction
...
class IonCNNDenovo(object):
    ...
    def _beam_search(self, model_wrapper: InferenceModelWrapper,
        ...
        if direction == Direction.forward:
            def get_start_mass(x): return x.prefix_mass
            first_label = config.GO_ID
            last_label = config.EOS_ID
        elif direction == Direction.backward:
            def get_start_mass(x): return x.suffix_mass
        ...
        batch_spectrum_representation = batch_denovo_data.spectrum_representation.to(
            device)
        ...
        for feature_index in range(num_features):
            ...
            spectrum_state = (
                batch_peak_location[feature_index], batch_peak_intensity[feature_index])

            if config.use_lstm:
                lstm_state = (initial_hidden_state_tuple[0][:, feature_index, :],
                              initial_hidden_state_tuple[1][:,
                                                            feature_index, :]
                              )
            else:
                lstm_state = None
        ...
        while True:
            ...
            for entry_index, search_entry in enumerate(active_search_list):
                ...
                for path in current_path_list:
                    ...
                    if not block_ion_location_param:
                        block_ion_location_param = [[precursor_mass], [
                            aa_seq_mass], direction_cint_map[direction]]
                    else:
                        block_ion_location_param[0].append(precursor_mass)
                        block_ion_location_param[1].append(aa_seq_mass)

                    residual_mass = precursor_mass - \
                        aa_seq_mass - config.mass_ID[last_label]
                    knapsack_tolerance = int(
                        round(peak_mass_tolerance * config.KNAPSACK_AA_RESOLUTION))
                    knapsack_candidates = self.knapsack_searcher.search_knapsack(
                        residual_mass, knapsack_tolerance)
            ...
            block_peak_location = torch.stack(
                block_peak_location, dim=0).contiguous()
            block_peak_intensity = torch.stack(
                block_peak_intensity, dim=0).contiguous()
            if config.use_lstm:
            ...
            for entry_index, search_entry in enumerate(active_search_list):
                ...
                for index in range(block_index, block_index + search_entry_size[entry_index]):
                    for aa_id in block_knapsack_candidates[index]:
                        if aa_id > 2:
                            ...
                            new_score_list = block_score_list[index] + [
                                current_log_prob[index][aa_id]]
                            new_score_sum = block_score_sum[index] + \
                                current_log_prob[index][aa_id]
                        else:
                        ...
                        if config.use_lstm:
                            new_path_state_tuple = (
                                new_state_tuple[0][:, index, :], new_state_tuple[1][:, index, :])
                        else:
                            new_path_state_tuple = None

                        new_path = SearchPath(
                            aa_id_list=block_aa_id_list[index] + [aa_id],
                            aa_seq_mass=block_aa_seq_mass[index] +
                            config.mass_ID[aa_id],
                ...
                if len(new_path_list) > self.beam_size:
                    ...
                    top_k_index = np.argpartition(-new_path_score,
                                                  self.beam_size)[:self.beam_size]
                    search_entry.current_path_list = [
                        new_path_list[ii] for ii in top_k_index]
                else:
    ...
    def _select_path(batch_denovo_data: BatchDenovoData, top_candidate_batch: list) -> list:
        ...
        for feature_index in range(feature_batch_size):
            ...
            if not candidate_list:
                ...
                best_beam_search_sequence = max(
                    candidate_list, key=lambda x: x.score)
    ...
    def _search_denovo_batch(self, batch_denovo_data: BatchDenovoData, model_wrapper: InferenceModelWrapper) -> list:
        ...
        for start_points in start_points_tuple:
            beam_search_result_batch = self._beam_search(
                model_wrapper, batch_denovo_data, start_points)
            for feature_index in range(feature_batch_size):
                top_candidate_batch[feature_index].extend(
                    beam_search_result_batch[feature_index])
        predicted_batch = self._select_path(
            batch_denovo_data, top_candidate_batch)
    ...
    def search_denovo(self, model_wrapper: InferenceModelWrapper,
        ...
        logger.info(f"GPU {dist.get_rank()}: start beam search denovo")
        predicted_denovo_list = []
        total_batch_num = int(
            len(beam_search_reader.dataset) / config.batch_size)
        for index, batch_denovo_data in enumerate(beam_search_reader):
            logger.info(
                f"GPU {dist.get_rank()}: Read {index} th/{total_batch_num//config.num_GPUs} batches.")
            predicted_batch = self._search_denovo_batch(
                batch_denovo_data, model_wrapper)
            predicted_denovo_list += predicted_batch
            for denovo_result in predicted_batch:
                denovo_writer.write(denovo_result.dda_feature,
                                    denovo_result.best_beam_search_sequence)
